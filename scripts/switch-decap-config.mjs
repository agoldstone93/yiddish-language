...
const baseConfig = load(fs.readFileSync(basePath, 'utf8'));

// For local dev, merge with local override if it exists
if (isLocal && fs.existsSync(localOverridePath)) {
  const localOverride = load(fs.readFileSync(localOverridePath, 'utf8'));
  const merged = { ...baseConfig, ...localOverride };
  fs.writeFileSync(outputPath, dump(merged, { lineWidth: -1 }), 'utf8');
